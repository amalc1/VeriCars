...
const AddCarForm = () => {
  ...
  const {
    data: processImgResult,
    error: processImgError,
    fn: processImgFn,
    loading: processImgLoading,
  ...
  const processWithAi = async () => {
    ...
    await processImgFn(uploadedAiImage as File);
  };

  useEffect(() => {
    if (processImgError) {
      toast.error(processImgError.message || "Failed to process image");
    }
  }, [processImgError]);

  useEffect(() => {
    if (processImgResult?.success) {
      const carDetails = processImgResult.data;
      ...
      if (!uploadedAiImage) {
        toast.error("No AI image to upload");
        return;
      }
      ...
      toast.success("Successfully extracted car details", {
        description: `Detected ${carDetails.year} ${carDetails.make} ${
          carDetails.model
        } with ${Math.round(carDetails.confidence * 100)}% accuracy`,
      });

      // Switch to manual tab for the user to review and fill in missing details
      setActiveTab("manual");
    }
  }, [processImgResult, uploadedAiImage, setValue]);
                    ...
                    <div className="flex flex-col items-center ">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button
                          ...
                          disabled={Boolean(processImgLoading)}
                        >
                          {processImgLoading ? (
                            <>
                              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
